define(['initialize'], function (initialize) {
  'use strict';
  initialize.controller('RiwayatLaluRevCtrl', ['$rootScope', '$scope', 'ModelItem', '$state', 'MedifirstService', 'CacheHelper',
    function ($rootScope, $scope, ModelItem, $state,  medifirstService, cacheHelper) {
                ...
                if ($scope.dataRiwayatLalu[i].objectfk == "RLU-000001") {
                  ...
                  for (var k = 0; k < $scope.listRiwayatPenyakit.length; k++) {
                    if ($scope.listRiwayatPenyakit[k].id == $scope.dataRiwayatLalu[i].nilai) {
                      $scope.item.riwayatPenyakit = $scope.listRiwayatPenyakit[k].id
                    }
                  }
                  ...
                  for (var k = 0; k < $scope.listGolonganDarah.length; k++) {
                    if ($scope.listGolonganDarah[k].id == $scope.dataRiwayatLalu[i].nilai) {
                      $scope.item.golonganDarahIbu = $scope.listGolonganDarah[k].id
                    }
                  }
                  ...
                  for (var k = 0; k < $scope.listGolonganDarah.length; k++) {
                    if ($scope.listGolonganDarah[k].id == $scope.dataRiwayatLalu[i].nilai) {
                      $scope.item.golonganDarahAyah = $scope.listGolonganDarah[k].id
                    }
                  }
      ...
      $scope.Save = function () {
        ...
        data = [{
          ...
          objectfk: "RLU-000001",
          nilai: $scope.item.riwayatPenyakit !== undefined ? $scope.item.riwayatPenyakit: null,
          satuan: "",
          jenisobject: "radio"
        },
        {
        ...
        },
        {
          ...
          objectfk: "RLU-000003",
          nilai: $scope.item.golonganDarahIbu !== undefined ? $scope.item.golonganDarahIbu:null,
          satuan: "",
          jenisobject: 'radio'
        },
        {
        ...
        },
        {
          ...
          objectfk: "RLU-000005",
          nilai: $scope.item.golonganDarahAyah !== undefined ? $scope.item.golonganDarahAyah:null,
          satuan: "",
          jenisobject: "radio"
        },
        {
        ...
        medifirstService.post('emr/save-data-rekam-medis', jsonSave).then(function (e) {
          medifirstService.postLogging('Pengkajian Keperawatan', 'Norec Antrian Pasien Diperiksa', $state.params.noRec, 'Riwayat Lalu').then(function (res) {
          })
        });
      }
      $scope.Back = function () {
