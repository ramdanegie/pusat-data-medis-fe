define(['initialize'], function (initialize) {
    ...
    initialize.controller('SkriningKekhususanRiwayatKelahiranRevCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
                                ...
                                if ($scope.dataRiwayatKel[i].objectfk == "RKL-000001") {
                                    ...
                                    for (var k = 0; k < $scope.listKelahiran.length; k++) {
                                        if ($scope.listKelahiran[k].id == $scope.dataRiwayatKel[i].nilai) {
                                            $scope.item.riwayatKelahiran = $scope.listKelahiran[k].id
                                        }
                                    }
            ...
            $scope.Save = function () {
                    ...
                    var data = [
                        {
                            ...
                            objectfk: "RKL-000001",
                            nilai: $scope.item.riwayatKelahiran !== undefined ? $scope.item.riwayatKelahiran :null,
                            satuan: "",
                            jenisobject: "radio"
                        },
                        {
                ...
                medifirstService.post('emr/save-data-rekam-medis', jsonSave).then(function (e) {
                    medifirstService.postLogging('Pengkajian Keperawatan', 'Norec Antrian Pasien Diperiksa', $state.params.noRec, 'Riwayat Kelahiran').then(function (res) {
                    })
                });
            }
