define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('LembarPenilaianPansEC3Ctrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            $scope.cetakPdf = function () {
                ...
                client.get('http://127.0.0.1:1237/printvb/e-rekammedis?cetak-emr-asesmen-pansssingkat&id='
                 + $scope.cc.nocm + '&emr=' + norecEMR + '&view=true' + '&idemr=' + 313  , function (response) {
            ...
            medifirstService.get("emr/get-rekam-medis-dynamic?emrid=" + 313).then(function (e) {
                // debugger
            ...
            if (chacePeriode != undefined) {
                ...
                medifirstService.get("emr/get-emr-transaksi-detail?noemr=" + nomorEMR + "&emrfk=" + $scope.cc.emrfk, true).then(function (dat) {
                    $scope.item.obj = []
                    $scope.item.obj2 = []
                    $scope.item.obj[400061]=$scope.now
                    $scope.item.obj[400062]={ value: $scope.cc.iddpjp, text: $scope.cc.dokterdpjp }
                    dataLoad = dat.data.data
                    for (var i = 0; i <= dataLoad.length - 1; i++) {
                        if (parseFloat($scope.cc.emrfk) == dataLoad[i].emrfk) {
                            if (dataLoad[i].type == "textbox") {
                                $scope.item.obj[dataLoad[i].emrdfk] = dataLoad[i].value
                                 if (dataLoad[i].emrdfk=='400060') 
