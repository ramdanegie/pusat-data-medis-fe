define(['initialize'], function (initialize) {
	'use strict';
	initialize.controller('ResumeRJCtrl', ['$q', '$scope', '$state', 'MedifirstService', '$timeout', 'CacheHelper', '$rootScope',
		function ($q, $scope, $state, medifirstService, $timeout, cacheHelper, $rootScope) {
			...
			function hapus(e) {
				...
				medifirstService.post('emr/post-resume-medis/delete', itemDelete).then(function (e) {
					if (e.status == 201) {
						init()
					}
				})
			...
			function init() {
				...
				$q.all([
					medifirstService.get("emr/get-resume-medis/" + $scope.nocm)
				]).then(function (res) {
			...
			$scope.Save = function (data) {
				...
				medifirstService.post('emr/post-resume-medis/save', item).then(function (e) {
					// delete $scope.item;
					clear()
					init();
					medifirstService.postLogging('Summary List', 'Norec resumemedis_t', e.data.resume.norec, 'Resume RJ').then(function (res) {
					})
				});
			};
			function clear() {
