define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('AsesmenMedisWajibLaporCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            medifirstService.getPart('emr/get-datacombo-perkawinan', true, true, 20).then(function (data) {
                $scope.listPerkawinan = data
                
            })
            medifirstService.getPart('emr/get-datacombo-pekerjaan', true, true, 20).then(function (data) {
                $scope.listPekerjaan = data
                
            })
             medifirstService.getPart('emr/get-datacombo-pendidikan', true, true, 20).then(function (data) {
            ...
            if (chacePeriode != undefined) {
                ...
                medifirstService.get("emr/get-emr-transaksi-detail?noemr=" + nomorEMR + "&emrfk=" + $scope.cc.emrfk, true).then(function (dat) {
                    // var durasi='';
                    //     if (datas[i].tglregistrasi!=null){
                    //         var tgldaftar = new Date(dat[i].tglregistrasi);
                    //         // if (datas[i].noreservasi != null){
                    //         //     var tgldaftar = new Date(datas[i].tanggalreservasi);
                    //         // } else {
                    //         //     var tgldaftar = new Date(datas[i].tglregistrasi);
                    //         // }
                    //         var tglpanggil = $scope.now;
                    //         var diff = Math.abs(tglpanggil - tgldaftar);
                    //         var durasi = Math.round((diff/1000)/60) + ' menit';
                    //         dat[i].tgldipanggildokter = new moment(tglpanggil).format('DD-MM-YYYY HH:mm')
                    //     }
                    //     datas[i].durasi= durasi;

                    $scope.item.obj = []
                    $scope.item.obj2 = []
                    $scope.item.obj[101167] = { value:peagawaiLogin.id,text:peagawaiLogin.namaLengkap}
            ...
            $scope.Save = function () {
                ...
                medifirstService.post('emr/save-emr-dinamis', jsonSave).then(function (e) {
                    ...
                    'Seleksi Pasien Instalasi Rehabilitasi Psikososial ' + ' dengan No EMR - ' + e.data.data.noemr + ' pada No Registrasi '
