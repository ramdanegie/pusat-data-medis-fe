define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('InformasiIbuRevCtrl', ['$rootScope', '$scope', 'ModelItem', '$state', 'DateHelper', '$timeout', 'MedifirstService', 'CacheHelper',
        function ($rootScope, $scope, ModelItem, $state, dateHelper, $timeout, medifirstService, cacheHelper) {
            ...
            function searchIbu() {
                ...
                medifirstService.get("emr/get-pasien?" +
                    tglLahirs +
                    rm +
                    pasien +
                    ayah +
            ...
            $scope.PilihIbu = function () {
                ...
                if ($scope.item.noCmIbu !== undefined) {
                    medifirstService.get("emr/get-info-pasien?noCm=" + $scope.item.noCmIbu).then(function (e) {
                        ...
                        if (result.statusrawat == 'dirawat') {
                            $scope.item.ibuDirawat = $scope.listIbuDirawat[0].id
                        } else {
                            $scope.item.ibuDirawat = $scope.listIbuDirawat[1].id
                        }
                                ...
                                if ($scope.dataInformasiIbu[i].objectfk == "IIB-000001") {
                                    ...
                                    for (var k = 0; k < $scope.listIbuDirawat.length; k++) {
                                        if ($scope.listIbuDirawat[k].id == $scope.dataInformasiIbu[i].nilai) {
                                            $scope.item.ibuDirawat = $scope.listIbuDirawat[k].id
                                        }
                                    }
                                    ...
                                    for (var k = 0; k < $scope.statusPerkawinan.length; k++) {
                                        if ($scope.statusPerkawinan[k].id == $scope.dataInformasiIbu[i].nilai) {
                                            $scope.item.statusPerkawinan = $scope.statusPerkawinan[k].id
                                        }
                                    }
                                    ...
                                    for (var k = 0; k < $scope.diTolongOleh.length; k++) {
                                        if ($scope.diTolongOleh[k].id == $scope.dataInformasiIbu[i].nilai) {
                                            $scope.item.ditolongOleh = $scope.diTolongOleh[k].id
                                        }
                                    }
                                }
            ...
            $scope.Save = function () {
                ...
                if ($scope.dataInformasiIbu.length != 0) {
                    var data = [
                        ...
                        },
                        {
                            ...
                            objectfk: "IIB-000002",
                            nilai: $scope.item.ibuDirawat !== undefined ? $scope.item.ibuDirawat :null,
                            satuan: "",
                            jenisobject: 'radio'
                        },
                        {
                        ...
                        },
                        {
                            ...
                            objectfk: "IIB-000008",
                            nilai: $scope.item.statusPerkawinan !== undefined ? $scope.item.statusPerkawinan : null,
                            satuan: "",
                            jenisobject: "radio"
                        },
                        {
                            ...
                            objectfk: "IIB-000009",
                            nilai: $scope.item.ditolongOleh !== undefined ? $scope.item.ditolongOleh: null,
                            satuan: "",
                            jenisobject: "radio"
                        }
                    ]
                ...
                medifirstService.post('emr/save-data-rekam-medis', jsonSave).then(function (e) {
                    medifirstService.postLogging('Pengkajian Keperawatan', 'Norec Antrian Pasien Diperiksa', $state.params.noRec, 'Informasi Ibu').then(function (res) {
                    })
                });
            };
