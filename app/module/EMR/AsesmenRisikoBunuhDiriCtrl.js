define(['initialize'], function (initialize) {
    'use strict';
    initialize.controller('AsesmenRisikoBunuhDiriCtrl', ['$q', '$rootScope', '$scope', 'ModelItem', '$state', 'CacheHelper', 'DateHelper', 'MedifirstService',
        function ($q, $rootScope, $scope, ModelItem, $state, cacheHelper, dateHelper, medifirstService) {
            ...
            medifirstService.get("emr/get-rekam-medis-dynamic?emrid=" + 181).then(function (e) {
            ...
            medifirstService.get("emr/get-emr-transaksi-detail?noemr=" + nomorEMR + "&emrfk=" + $scope.cc.emrfk, true).then(function (dat) {
                $scope.item.obj = []
                $scope.item.obj2 = []
                $scope.item.obj[20000]=$scope.cc.tglregistrasi
                $scope.item.obj[20002]={ value: $scope.cc.iddpjp, text: $scope.cc.dokterdpjp }
                $scope.item.obj[20001]={value: $scope.cc.objectruanganfk, text : $scope.cc.namaruangan}
                ...
                medifirstService.get("emr/get-nilai-statis-igd?noregistrasi=" + $scope.cc.noregistrasi + "&objectidawal=2994&objectidakhir=3014&idemr=123", true).then(function (datas) {
                    if (datas.data.data.length>0){
                        $scope.item.obj[20389]=datas.data.data[0].nilai
                    }
                })
